<?php
namespace kkse\quick\http;

/**
 * curl http请求
 * Class CurlHttp
 * @package kkse\quick\http
 */
class CurlHttp
{
    ...
    protected $redirect = false;//自动转义302
    ...
    {
        if ($cookiefile) {
            $this->option[CURLOPT_COOKIEJAR] = $cookiefile;
            $this->option[CURLOPT_COOKIEFILE] = $cookiefile;
        } else {
            unset($this->option[CURLOPT_COOKIEJAR]);
    ...
    public function setUseragent($useragent)
    {
        if ($useragent) {
            ...
            unset($this->option[CURLOPT_USERAGENT]);
        }
        return $this;
    }
    ...
    public function setCallback($callback)
    {
        if ($callback) {
            ...
            unset($this->option['callback']);
        }
        return $this;
    }

    public function setSsl_verifypeer($ssl_verifypeer)
    {
        $this->ssl[CURLOPT_SSL_VERIFYPEER] = (bool)$ssl_verifypeer;
        return $this;
    }

    public function setSsl_verifyhost($ssl_verifyhost)
    {
        $this->ssl[CURLOPT_SSL_VERIFYHOST] = intval($ssl_verifyhost);
        return $this;
    }
    ...
    {
        if ($sslcert) {
            $this->ssl[CURLOPT_SSLCERTTYPE] = $type;
            $this->ssl[CURLOPT_SSLCERT] = $sslcert;
        } else {
            unset($this->ssl[CURLOPT_SSLCERTTYPE]);
            unset($this->ssl[CURLOPT_SSLCERT]);
        }
    ...
    {
        if ($sslkey) {
            $this->ssl[CURLOPT_SSLKEYTYPE] = $type;
            $this->ssl[CURLOPT_SSLKEY] = $sslkey;
        } else {
            unset($this->ssl[CURLOPT_SSLKEYTYPE]);
            unset($this->ssl[CURLOPT_SSLKEY]);
        }
    ...
    public function clean($all = false){
        ...
        $this->postdata = null;
        $this->postmode = 0;
        $all and $this->option = self::$doption;
        return $this;
    }
    ...
    public function request()
    {
        ...
        if ($this->headers) {
            ...
            $reqopt[CURLOPT_HTTPHEADER] = $header_arr;
        }
        ...
        if (!$this->postmode && $this->redirect) {
            if ($ret['errno'] == 0 && $ret['info']['http_code'] == 302) {
                $this->url = $ret['info']['redirect_url'];
                return $this->request();
            }
        }

        if (!empty($this->option['callback']) && is_callable($this->option['callback'])) {
            $cp_result = $result;
            call_user_func($this->option['callback'], $cp_result, $this);//禁止回调函数修改返回值
        }
