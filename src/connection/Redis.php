...
class Redis extends \Redis
{
    ...
    protected $select = 0;//使用select函数修改
    protected $prefix = '';//使用setOption(Redis::OPT_PREFIX, '')函数修改

    public function __construct(string $host, string $port, bool $persistent = false, $auth = '')
    {
    ...
    public function scan( &$iterator, $pattern = null, $count = 0 ) {
        return $this->initHandler()->scan($iterator, $pattern, $count);
    }
    ...
    public function watch( $key ) {
        call_user_func_array([$this->initHandler(), __FUNCTION__], func_get_args());
    }
